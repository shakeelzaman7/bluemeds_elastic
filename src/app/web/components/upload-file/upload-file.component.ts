...
export class UploadFileComponent implements OnInit, AfterViewInit {
  ...
  typeValidation(type) {
    ...
    if (type == 'application/pdf' || splitType == 'image' || splitType == 'video' || type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
  ...
  sizeValidation(size) {
    // el archivo no puede ser mayor a 10 MB
  ...
  uploadFiles() {
    ...
    this.files.forEach((file) => {
      ...
      formData.append('document_name', 'authorization');
      http.onload = async () => {
      ...
      http.open('POST', this.api.url + '/' + `profile/deliveries/${this.deliveryId}/insurance-attachments`, true);
